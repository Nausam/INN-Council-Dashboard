...
export function LeaveRequestPageView({
  initialData,
}: {
  initialData?: Awaited<ReturnType<typeof fetchUserLeaveRequests>>;
}) {
  const { currentUser: authUser } = useUser();
  const currentUser: CurrentUser = authUser
    ? { fullName: authUser.fullName, email: authUser.email, accountId: authUser.id }
    : null;
