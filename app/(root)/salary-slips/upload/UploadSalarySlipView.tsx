...
export function UploadSalarySlipView({
  initialEmployees,
}: {
  ...
  const canPreviewBatchMatches = slipMatchEmployees.length > 0;
  ...
  const batchPreview = useMemo(() => {
    ...
    return batchFiles.map((file) => {
      ...
      if (!canPreviewBatchMatches) {
        return {
          fileName: file.name,
          status: "pending" as const,
          error: "Employee matching will be checked during upload",
          detail:
            "The employee list is not available in this browser preview, but the server can still match by employee name or record card.",
        };
      }
  ...
  }, [batchFiles, canPreviewBatchMatches, slipMatchEmployees]);

  const batchReadyCount = canPreviewBatchMatches
    ? batchPreview.filter((row) => row.status === "ok").length
    : batchPreview.length;
  ...
  const handleBatchSubmit = async (e: React.FormEvent) => {
    ...
    if (canPreviewBatchMatches && batchReadyCount === 0) {
      setBatchError({
